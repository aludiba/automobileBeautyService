...
@implementation MBProgressHUD (PBNJ)
#pragma mark 显示信息
+ (void)PBshow:(NSString *)text icon:(NSString *)icon view:(UIView *)view
{
...
+ (void)PBshowError:(NSString *)error toView:(UIView *)view{
    [self PBshow:error icon:@"error.png" view:view];
}

+ (void)PBshowSuccess:(NSString *)success toView:(UIView *)view
{
    [self PBshow:success icon:@"success.png" view:view];
}
...
{
    [self PBshowSuccess:success toView:nil];
}

+ (void)PBshowError:(NSString *)error
{
    [self PBshowError:error toView:nil];
}

+ (void)PBshowReminderText:(NSString *)reminderText {
    [self PBshowReminderText:reminderText toView:nil];
}

+ (MBProgressHUD *)auto4BeushowMessage:(NSString *)message
{
    return [self PBshowMessage:message toView:nil];
}
