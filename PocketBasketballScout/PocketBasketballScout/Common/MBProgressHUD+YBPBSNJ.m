...
@implementation MBProgressHUD (YBPBSNJ)
#pragma mark 显示信息
+ (void)YBPBSshow:(NSString *)text icon:(NSString *)icon view:(UIView *)view
{
...
+ (void)YBPBSshowError:(NSString *)error toView:(UIView *)view{
    [self YBPBSshow:error icon:@"error.png" view:view];
}

+ (void)YBPBSshowSuccess:(NSString *)success toView:(UIView *)view
{
    [self YBPBSshow:success icon:@"success.png" view:view];
}
...
{
    [self YBPBSshowSuccess:success toView:nil];
}

+ (void)YBPBSshowError:(NSString *)error
{
    [self YBPBSshowError:error toView:nil];
}

+ (void)YBPBSshowReminderText:(NSString *)reminderText {
    [self YBPBSshowReminderText:reminderText toView:nil];
}

+ (MBProgressHUD *)auto4BeushowMessage:(NSString *)message
{
    return [self YBPBSshowMessage:message toView:nil];
}
