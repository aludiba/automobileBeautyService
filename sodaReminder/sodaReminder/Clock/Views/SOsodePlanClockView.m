...
@interface SOsodePlanClockView()<UITextFieldDelegate>
@property(nonatomic, strong)UIView *backView;
@property(nonatomic, strong)UIView *contentView;
@property(nonatomic, strong)UIButton *saveButton;
@end

@implementation SOsodePlanClockView
- (instancetype)init
{
    self = [super init];
    if (self) {
...
- (void)btnClick{
    self.hidden = YES;
    if (self.SOsodePlanClockSaveB) {
        self.SOsodePlanClockSaveB(self);
    }
}
...
- (UIView *)backView{
    if (!_backView) {
        ...
        _backView.backgroundColor = SOH_Color(0, 0, 0, 0.5);
    }
    return _backView;
}
- (UIView *)contentView{
