...
@implementation MBProgressHUD (EBNJ)
#pragma mark 显示信息
+ (void)EBshow:(NSString *)text icon:(NSString *)icon view:(UIView *)view
{
...
+ (void)EBshowError:(NSString *)error toView:(UIView *)view{
    [self EBshow:error icon:@"error.png" view:view];
}

+ (void)EBshowSuccess:(NSString *)success toView:(UIView *)view
{
    [self EBshow:success icon:@"success.png" view:view];
}
...
{
    [self EBshowSuccess:success toView:nil];
}

+ (void)EBshowError:(NSString *)error
{
    [self EBshowError:error toView:nil];
}

+ (void)EBshowReminderText:(NSString *)reminderText {
    [self EBshowReminderText:reminderText toView:nil];
}

+ (MBProgressHUD *)auto4BeushowMessage:(NSString *)message
{
    return [self EBshowMessage:message toView:nil];
}
