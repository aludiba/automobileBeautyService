//
//  SDPasswordChangeViewController.m
//  Simple Diary
//
...
@interface SDPasswordChangeViewController ()<UITextFieldDelegate>
...
@implementation SDPasswordChangeViewController
...
- (void)btnClick:(UIButton *)sender{
    ...
    if (!self.currentPassword.length) {
        [MBProgressHUD SDshowReminderText:NSLocalizedString(@"请输入新密码", nil)];
        return;
    }
    if (!self.confirmPassword.length) {
        [MBProgressHUD SDshowReminderText:NSLocalizedString(@"请确认密码", nil)];
        return;
    }
