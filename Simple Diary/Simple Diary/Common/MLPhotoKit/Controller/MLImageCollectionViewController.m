...
- (void)viewDidLoad {
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"返回", nil) style:UIBarButtonItemStylePlain target:self action:@selector(cancelNacigationItemAction)];
}
...
- (void)changeRightBarButtonItemTitle
{
    if (self.selectArray.count > 0) {
        [self setRightBarButtonItemWithTitle:[NSString stringWithFormat:@"%@(%ld)", NSLocalizedString(@"确定", nil),self.selectArray.count]];
    }
    else
    {
        [self setRightBarButtonItemWithTitle:NSLocalizedString(@"取消", nil)];
    }
}
...
- (void)didClickNavigationBarViewRightButton
{
    ...
    for (int i = 0; i < self.navigationController.childViewControllers.count; i
         ++) {
        id obj = self.navigationController.childViewControllers[i];
        if ([obj isKindOfClass:[SDWriteDiaryViewController class]]) {
...
- (void)closeButtonAction:(UIButton *)sender
{
    ...
    if (sender.selected) {
        ...
        if (self.selectArray.count >= self.maxCount) {
            [MLPhotoImageHelper showAlertWithTittle:NSLocalizedString(@"最多只能选择6张图片", nil) message:nil showController:self isSingleAction:YES complete:nil];
            return;
        }
...
//                    if ([collection.localizedTitle isEqualToString:NSLocalizedString(@"相机胶卷", nil)] || [collection.localizedTitle isEqualToString:@"Camera Roll"]) {
