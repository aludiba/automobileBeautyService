...
- (void)viewDidLoad {
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"取消", nil) style:UIBarButtonItemStylePlain target:self action:@selector(dismissController)];
...
- (void)loadAlbums
{
    if (![MLPhotoImageHelper isOpenAuthority]) {
        [MLPhotoImageHelper showAlertWithTittle:NSLocalizedString(@"您未打开照片权限", nil) message:nil showController:self isSingleAction:NO complete:^(NSInteger index) {
            if (index == 1) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        cell.albumTitleLabel.text = NSLocalizedString(@"相机胶卷", nil);
