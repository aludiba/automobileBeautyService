//
//  PKPasswordChangeViewController.m
//  PocketBooks
//
...
@interface PKPasswordChangeViewController ()<UITextFieldDelegate>
...
@implementation PKPasswordChangeViewController
...
- (void)btnClick:(UIButton *)sender{
    ...
    if (!self.oldPassword.length) {
        [MBProgressHUD PKshowReminderText:NSLocalizedString(@"请输入旧密码", nil)];
        return;
    }
    if (!self.currentPassword.length) {
        [MBProgressHUD PKshowReminderText:NSLocalizedString(@"请输入新密码", nil)];
        return;
    }
    if (!self.confirmPassword.length) {
        [MBProgressHUD PKshowReminderText:NSLocalizedString(@"请确认密码", nil)];
        return;
    }
    if (![self.currentPassword isEqualToString:self.confirmPassword]) {
        [MBProgressHUD PKshowReminderText:NSLocalizedString(@"两次密码输入不一致", nil)];
        return;
    }
            ...
            if (isSuccessful) {
                ...
                [BmobUser loginInbackgroundWithAccount:name andPassword:self.confirmPassword block:^(BmobUser *user, NSError *error) {
                    if (error) {
                        ...
                        [MBProgressHUD PKshowReminderText:NSLocalizedString(@"密码修改成功", nil)];
