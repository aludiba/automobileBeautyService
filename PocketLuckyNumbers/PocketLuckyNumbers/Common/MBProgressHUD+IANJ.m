...
@implementation MBProgressHUD (IANJ)
#pragma mark 显示信息
+ (void)IAshow:(NSString *)text icon:(NSString *)icon view:(UIView *)view
{
...
+ (void)IAshowError:(NSString *)error toView:(UIView *)view{
    [self IAshow:error icon:@"error.png" view:view];
}

+ (void)IAshowSuccess:(NSString *)success toView:(UIView *)view
{
    [self IAshow:success icon:@"success.png" view:view];
}
...
{
    [self IAshowSuccess:success toView:nil];
}

+ (void)IAshowError:(NSString *)error
{
    [self IAshowError:error toView:nil];
}

+ (void)IAshowReminderText:(NSString *)reminderText {
    [self IAshowReminderText:reminderText toView:nil];
}

+ (MBProgressHUD *)auto4BeushowMessage:(NSString *)message
{
    return [self IAshowMessage:message toView:nil];
}
