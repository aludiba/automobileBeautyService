//
//  YWKGAgamePlanDateTableViewCell.m
//  wineAwake
//
...
@interface YWKGAgamePlanDateTableViewCell()<UITextFieldDelegate>
...
@end
@implementation YWKGAgamePlanDateTableViewCell
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        [self.editText mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.trailing.equalTo(self.rightArrowImgView.mas_leading);
...
- (void)dateResult:(UIBarButtonItem *)sender{
    if (sender.tag == 100) {
        ...
        if (dateStr.length) {
            self.model.YWKGAcontent = dateStr;
        }else{
            self.editText.placeholder = _model.YWKGAdefaultString;
        }
        if (self.YWKGAgamePlanDateB) {
            self.YWKGAgamePlanDateB(self);
        }
...
- (UIDatePicker *)datePicker{
    if (!_datePicker) {
        ...
        if (!language) {
            language = [YWKGAHBTool getPreferredLanguage];
...
- (UIToolbar *)toolbar{
    if (!_toolbar) {
        ...
        toolbar.barTintColor = YWKGAH_Color(242, 242, 242, 1);
        //设置工具条的frame
        toolbar.frame = CGRectMake(0, 0, YWKGAWIDTH, 44);
        //给工具条添加按钮
...
- (UIImageView *)rightArrowImgView{
    if (!_rightArrowImgView) {
        _rightArrowImgView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"YWKGA_arrow_right"]];
    }
