...
@interface GAgamePlanDateTableViewCell()<UITextFieldDelegate>
...
- (void)setModel:(GAgamePlanModel *)model{
    _model = model;
    self.titleLbl.text = _model.GAtitle;
    if (_model.GAcontent.length) {
        self.editText.text = _model.GAcontent;
    }else{
        self.editText.placeholder = _model.GAdefaultString;
    }
}
...
- (void)dateResult:(UIBarButtonItem *)sender{
    if (sender.tag == 100) {
        ...
        if (self.GAgamePlanDateB) {
            self.GAgamePlanDateB(self);
        }
...
- (UIDatePicker *)datePicker{
    if (!_datePicker) {
        ...
        if (!language) {
            language = [GAHBTool getPreferredLanguage];
...
- (UIToolbar *)toolbar{
    if (!_toolbar) {
        ...
        toolbar.barTintColor = GAH_Color(242, 242, 242, 1);
        //设置工具条的frame
        toolbar.frame = CGRectMake(0, 0, GAWIDTH, 44);
        //给工具条添加按钮
