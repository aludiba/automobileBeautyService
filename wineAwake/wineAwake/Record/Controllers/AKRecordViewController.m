...
@interface AKRecordViewController ()<JTCalendarDelegate>{
...
@implementation AKRecordViewController
...
- (void)createRandomEvents{
    ...
    for(int i = 0; i < 30; ++i){
        ...
        NSString *key = [AKUIUtilities AKformattedTimeStringWithDate:randomDate format:@"yyyy-MM-dd"];
...
- (BOOL)haveEventForDay:(NSDate *)date{
    NSString *key = [AKUIUtilities AKformattedTimeStringWithDate:date format:@"yyyy-MM-dd"];
...
- (void)calendar:(JTCalendarManager *)calendar didTouchDayView:(JTCalendarDayView *)dayView{
    ...
    if ([_dateSelected compare:nowDate] != NSOrderedDescending) {
        NSString *dateSelectedString = [AKUIUtilities AKformattedTimeStringWithDate:_dateSelected format:@"yyyy-MM-dd"];
...
//        [self loadData:_dateSelected];
        if (contentDic) {
        AKClockRecordViewController *clockRecordVC = [[AKClockRecordViewController alloc] init];
        ...
        [self.navigationController pushViewController:clockRecordVC animated:YES];
            ...
            [MBProgressHUD AKshowReminderText:NSLocalizedString(@"这一天未打卡", nil)];
        }
    }
