...
@implementation MBProgressHUD (WANJ)
#pragma mark 显示信息
+ (void)WAshow:(NSString *)text icon:(NSString *)icon view:(UIView *)view
{
...
+ (void)WAshowError:(NSString *)error toView:(UIView *)view{
    [self WAshow:error icon:@"error.png" view:view];
}

+ (void)WAshowSuccess:(NSString *)success toView:(UIView *)view
{
    [self WAshow:success icon:@"success.png" view:view];
}
...
{
    [self WAshowSuccess:success toView:nil];
}

+ (void)WAshowError:(NSString *)error
{
    [self WAshowError:error toView:nil];
}

+ (void)WAshowReminderText:(NSString *)reminderText {
    [self WAshowReminderText:reminderText toView:nil];
}

+ (MBProgressHUD *)auto4BeushowMessage:(NSString *)message
{
    return [self WAshowMessage:message toView:nil];
}
