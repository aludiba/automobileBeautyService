//
//  WADrinkingPlanDateTableViewCell.m
//  wineAwake
//
...
@interface WADrinkingPlanDateTableViewCell()<UITextFieldDelegate>
...
@end
@implementation WADrinkingPlanDateTableViewCell
...
- (void)setModel:(WADrinkingPlanModel *)model{
    _model = model;
...
- (void)dateResult:(UIBarButtonItem *)sender{
    if (sender.tag == 100) {
        ...
        if (self.WADrinkingPlanDateB) {
            self.WADrinkingPlanDateB(self);
        }
...
- (UIDatePicker *)datePicker{
    if (!_datePicker) {
        ...
        if (!language) {
            language = [WAHBTool getPreferredLanguage];
...
- (UIToolbar *)toolbar{
    if (!_toolbar) {
        ...
        toolbar.barTintColor = WAH_Color(242, 242, 242, 1);
        //设置工具条的frame
        toolbar.frame = CGRectMake(0, 0, WAWIDTH, 44);
        //给工具条添加按钮
...
- (UIImageView *)rightArrowImgView{
    if (!_rightArrowImgView) {
        _rightArrowImgView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"WA_arrow_right"]];
    }
...
- (UIView *)line{
    if (!_line) {
        ...
        _line.backgroundColor = WAH_Color(242, 242, 242, 1);
    }
    return _line;
}
@end
