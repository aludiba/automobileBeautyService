...
@interface WADrinkingPlanClockView()<UITextFieldDelegate>
@property(nonatomic, strong)UIView *backView;
@property(nonatomic, strong)UIView *contentView;
@property(nonatomic, strong)UIButton *saveButton;
@end

@implementation WADrinkingPlanClockView
- (instancetype)init
{
    self = [super init];
    if (self) {
...
- (void)btnClick{
    self.hidden = YES;
    if (self.WADrinkingPlanClockSaveB) {
        self.WADrinkingPlanClockSaveB(self);
    }
}
...
- (UIView *)backView{
    if (!_backView) {
        ...
        _backView.backgroundColor = WAH_Color(0, 0, 0, 0.5);
    }
    return _backView;
}
- (UIView *)contentView{
