//
//  PWDrinkingPlanDateTableViewCell.m
//  wineAwake
//
...
@interface PWDrinkingPlanDateTableViewCell()<UITextFieldDelegate>
...
@end
@implementation PWDrinkingPlanDateTableViewCell
...
- (void)setModel:(PWDrinkingPlanModel *)model{
    _model = model;
...
- (void)dateResult:(UIBarButtonItem *)sender{
    if (sender.tag == 100) {
        ...
        if (dateStr.length) {
            self.model.content = dateStr;
        }else{
            self.editText.placeholder = _model.defaultString;
        }
        if (self.PWDrinkingPlanDateB) {
            self.PWDrinkingPlanDateB(self);
        }
...
- (UIDatePicker *)datePicker{
    if (!_datePicker) {
        ...
        if (!language) {
            language = [PWHBTool getPreferredLanguage];
...
- (UIToolbar *)toolbar{
    if (!_toolbar) {
        ...
        toolbar.barTintColor = PWH_Color(242, 242, 242, 1);
        //设置工具条的frame
        toolbar.frame = CGRectMake(0, 0, PWWIDTH, 44);
        //给工具条添加按钮
...
- (UIImageView *)rightArrowImgView{
    if (!_rightArrowImgView) {
        _rightArrowImgView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"PW_arrow_right"]];
    }
...
- (UIView *)line{
    if (!_line) {
        ...
        _line.backgroundColor = PWH_Color(242, 242, 242, 1);
    }
    return _line;
}
@end
