//
//  PWDrinkingPlanViewController.m
//  wineAwake
//
...
#import "PWDrinkingPlanModel.h"
#import "PWDrinkingPlanSaveModel.h"
#import "PWDrinkingPlanEditTableViewCell.h"
#import "PWDrinkingPlanDateTableViewCell.h"
#import "PWDrinkingPlanSaveTableViewCell.h"

@interface PWDrinkingPlanViewController ()<UITableViewDelegate,UITableViewDataSource>
@property(nonatomic, strong)NSMutableArray *viewDataArray;
@property(nonatomic, strong)UITableView *mainTable;
@end

@implementation PWDrinkingPlanViewController
...
- (void)setContentView{
    PWDrinkingPlanModel *wineAgesModel = [[PWDrinkingPlanModel alloc] init];
    ...
    PWDrinkingPlanModel *timeDrinkModel = [[PWDrinkingPlanModel alloc] init];
    timeDrinkModel.type = 1;
    timeDrinkModel.title = NSLocalizedString(@"戒酒时间", nil);
    if (self.objectId.length) {
        timeDrinkModel.content = [PWUIUtilities PWformattedTimeStringWithDate:self.saveModel.timeDrinkDate format:@"yyyy-MM-dd"];
    }
    timeDrinkModel.defaultString = NSLocalizedString(@"请选择戒酒时间", nil);
    [self.viewDataArray addObject:timeDrinkModel];
    
    PWDrinkingPlanModel *drinkEveryDayModel = [[PWDrinkingPlanModel alloc] init];
    ...
    PWDrinkingPlanModel *winePricesModel = [[PWDrinkingPlanModel alloc] init];
    ...
    PWDrinkingPlanModel *alcoholContentModel = [[PWDrinkingPlanModel alloc] init];
    ...
    PWDrinkingPlanModel *saveModel = [[PWDrinkingPlanModel alloc] init];
    saveModel.type = 2;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    PWDrinkingPlanModel *viewModel = self.viewDataArray[indexPath.row];
    if (viewModel.type == 0) {
        PWDrinkingPlanEditTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"PWDrinkingPlanEditTableViewCell" forIndexPath:indexPath];
        cell.model = viewModel;
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        return cell;
    }else if (viewModel.type == 1){
        PWDrinkingPlanDateTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"PWDrinkingPlanDateTableViewCell" forIndexPath:indexPath];
        cell.model = viewModel;
        __weak typeof(self) weakSelf = self;
        cell.PWDrinkingPlanDateB = ^(PWDrinkingPlanDateTableViewCell * _Nonnull cell) {
            [weakSelf.mainTable reloadData];
        };
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        return cell;
    }else{
        PWDrinkingPlanSaveTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"PWDrinkingPlanSaveTableViewCell" forIndexPath:indexPath];
        __weak typeof(self) weakSelf = self;
        cell.PWDrinkingPlanSaveB = ^(PWDrinkingPlanSaveTableViewCell * _Nonnull cell) {
            [weakSelf closeEdit];
            [weakSelf save];
        };
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
...
- (void)save{
    for (int i = 0; i < self.viewDataArray.count; i++) {
        PWDrinkingPlanModel *model = self.viewDataArray[i];
        if (!model.content.length && model.type != 2) {
            [MBProgressHUD PWshowReminderText:model.defaultString];
            return;
        }else{
            switch (i) {
                case 0:
...
//                    break;
    ...
    PWDrinkingPlanModel *model = self.viewDataArray[1];
    NSString *dateString = model.content;
    NSDate *date = [PWUIUtilities PWdateFromString:dateString formate:@"yyyy-MM-dd"];
...
- (UITableView *)mainTable{
    if (!_mainTable) {
        ...
        [_mainTable registerClass:[PWDrinkingPlanEditTableViewCell class] forCellReuseIdentifier:@"PWDrinkingPlanEditTableViewCell"];
        [_mainTable registerClass:[PWDrinkingPlanDateTableViewCell class] forCellReuseIdentifier:@"PWDrinkingPlanDateTableViewCell"];
        [_mainTable registerClass:[PWDrinkingPlanSaveTableViewCell class] forCellReuseIdentifier:@"PWDrinkingPlanSaveTableViewCell"];
