...
- (void)ZLSetContentView{
    ...
    [self.ZLBackView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(ZLHEIGHT / 3);
    }];
    [self.ZLImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(self.ZLBackView.mas_centerX).offset(-50);
        make.centerY.equalTo(self.ZLBackView);
        make.width.mas_equalTo(100);
        make.height.mas_equalTo(100);
    }];
    [self.ZLNickNameLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(self.ZLBackView.mas_centerX).offset(60);
        make.centerY.equalTo(self.ZLImageView);
    ...
    [self.ZLChangePasswordButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.ZLBackView.mas_bottom).offset(30);
        make.centerX.equalTo(self.view);
        make.width.mas_equalTo((ZLWIDTH - 60));
        make.height.mas_equalTo(60);
    }];
    [self.ZLSignOutButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.ZLChangePasswordButton.mas_bottom).offset(25);
        make.centerX.equalTo(self.view);
        make.width.mas_equalTo((ZLWIDTH - 60));
        make.height.mas_equalTo(60);
    }];
...
- (void)ZLNicknameUpdate{
    ...
    [MBProgressHUD ZLshowReminderText:NSLocalizedString(@"更新成功", nil)];
}
#pragma mark - 属性懒加载
...
- (UIView *)ZLBackView{
    if (!_ZLBackView) {
        ...
        _ZLBackView.backgroundColor = [UIColor systemPurpleColor];
    }
    return _ZLBackView;
}
...
- (UILabel *)ZLNickNameLabel{
    if (!_ZLNickNameLabel) {
        ...
        _ZLNickNameLabel.textColor = [UIColor cyanColor];
        _ZLNickNameLabel.font = [UIFont systemFontOfSize:20];
...
- (UITextField *)ZLNicknameTextField{
    if (!_ZLNicknameTextField) {
        ...
        _ZLNicknameTextField.textColor = [UIColor blackColor];
        _ZLNicknameTextField.font = [UIFont systemFontOfSize:22];
        _ZLNicknameTextField.backgroundColor = [UIColor greenColor];
        _ZLNicknameTextField.layer.cornerRadius = 3.0f;
...
- (UIButton *)ZLChangePasswordButton{
    if (!_ZLChangePasswordButton) {
        ...
        [_ZLChangePasswordButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        _ZLChangePasswordButton.titleLabel.font = [UIFont systemFontOfSize:25];
        _ZLChangePasswordButton.titleLabel.numberOfLines = 0;
        _ZLChangePasswordButton.backgroundColor = [UIColor systemGreenColor];
...
- (UIButton *)ZLSignOutButton{
    if (!_ZLSignOutButton) {
        ...
        [_ZLSignOutButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        _ZLSignOutButton.titleLabel.font = [UIFont systemFontOfSize:25];
     _ZLSignOutButton.titleLabel.numberOfLines = 0;
        _ZLSignOutButton.backgroundColor = [UIColor systemGreenColor];
