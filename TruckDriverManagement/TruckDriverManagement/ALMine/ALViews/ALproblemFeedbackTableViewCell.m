//
//  ALproblemFeedbackTableViewCell.m
...
@interface ALproblemFeedbackTableViewCell()<UITextViewDelegate>
...
@end
@implementation ALproblemFeedbackTableViewCell
...
//        [self.contentView addSubview:self.ALline];
        ...
        [self.ALeditableView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self.ALtitleLbl.mas_bottom).offset(15);
        ...
        [self.ALhiddenLbl mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self.ALeditableView).offset(6);
            
            make.leading.equalTo(self.ALeditableView).offset(6);
            make.trailing.equalTo(self.ALeditableView).offset(-6);
...
- (UILabel *)ALtitleLbl{
    if (!_ALtitleLbl) {
        ...
        _ALtitleLbl.font = [UIFont fontWithName:@"PingFangSC-Regular" size:18];
...
- (UILabel *)ALhiddenLbl{
    if (!_ALhiddenLbl) {
        ...
        _ALhiddenLbl.font = [UIFont fontWithName:@"PingFangSC-Regular" size:18];
...
- (UITextView *)ALeditableView{
    if (!_ALeditableView) {
        ...
        _ALeditableView.font = [UIFont fontWithName:@"PingFangSC-Regular" size:20];
        _ALeditableView.textColor = [UIColor colorWithRed:19/255.0 green:29/255.0 blue:50/255.0 alpha:1/1.0];
        _ALeditableView.layer.cornerRadius = 8.0f;
        _ALeditableView.layer.masksToBounds = YES;
        _ALeditableView.layer.borderColor = [UIColor grayColor].CGColor;
        _ALeditableView.layer.borderWidth = 1.5f;
