...
@interface ALGasolineViewController ()<UITableViewDelegate,UITableViewDataSource>
@property(nonatomic, strong)NSMutableArray *ALDataArray;//页面数据
@property(nonatomic, strong)UIButton *ALaddButton;
@end

@implementation ALGasolineViewController
...
- (void)ALLoadData{
    AVUser *author = [AVUser currentUser];
     AVQuery *bquery = [AVQuery queryWithClassName:@"ALGasoline"];
    ...
    [bquery findObjectsInBackgroundWithBlock:^(NSArray *array, NSError *error) {
               if (error) {
                   ...
                   for (int i = (int)(array.count - 1); i > -1; i--) {
                       ...
                       ALGasolineModel *ALmodel = [[ALGasolineModel alloc] init];
                       ...
                       ALmodel.ALGasStationLocation = [obj objectForKey:@"ALGasStationLocation"];
                       ALmodel.ALCostAmount = [[obj objectForKey:@"ALCostAmount"] integerValue];
                       ALmodel.ALFuelType = [obj objectForKey:@"ALFuelType"];
...
- (void)ALDeleteData:(ALGasolineModel *)model{
    AVObject *diary = [AVObject objectWithClassName:@"ALGasoline" objectId:model.ALobjectId];
...
- (void)ALaddAction:(UIButton *)sender{
    AVUser *bUser = [AVUser currentUser];
    if (bUser) {
        ALGasolineAddViewController *ALaddVC = [[ALGasolineAddViewController alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ALGasolineModel *ALmodel = self.ALDataArray[indexPath.row];
    ALGasolineTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"ALGasolineTableViewCell" forIndexPath:indexPath];
    cell.ALGasolinemodel = ALmodel;
...
- (UITableView *)ALmainTable{
    if (!_ALmainTable) {
        ...
        [_ALmainTable registerClass:[ALGasolineTableViewCell class] forCellReuseIdentifier:@"ALGasolineTableViewCell"];
