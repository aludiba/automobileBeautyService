...
@interface FBWardrobeViewController ()<UITableViewDataSource,UITableViewDelegate>
@property(nonatomic, strong)NSMutableArray *FBDataArray;
@property(nonatomic, strong)UIView *FBBackView;
@property(nonatomic, strong)UIImageView *FBcoatImgView;//外套
@property(nonatomic, strong)UIButton *FBaddButton;
@end

@implementation FBWardrobeViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.title = NSLocalizedString(@"衣柜", nil);
...
- (void)FBsetcontentView{
    ...
    [self.FBBackView addSubview:self.FBcoatImgView];
    [self.FBcoatImgView mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (void)FBLoadData{
    ...
    AVQuery *FBbquery = [AVQuery queryWithClassName:@"FBWardrobe"];
    ...
    [FBbquery findObjectsInBackgroundWithBlock:^(NSArray *array, NSError *error) {
               if (error) {
                   ...
                   for (int i = (int)(array.count - 1); i > -1; i--) {
                       ...
                       FBWardrobeModel *FBmodel = [[FBWardrobeModel alloc] init];
                       FBmodel.FBobjectId = [FBobj objectId];
                       FBmodel.FBtypes = [FBobj objectForKey:@"FBtypes"];
                       FBmodel.FBnumber = [FBobj objectForKey:@"FBnumber"];
                       FBmodel.FBstorage = [FBobj objectForKey:@"FBstorage"];
                       FBmodel.FBpurchasetime = [FBobj objectForKey:@"FBpurchasetime"];
                       FBmodel.FBpurchasetime = [FBobj objectForKey:@"FBpurchasetime"];
...
- (void)FBDeleteData:(FBWardrobeModel *)FBmodel{
    AVObject *FBdiary = [AVObject objectWithClassName:@"FBWardrobe" objectId:FBmodel.FBobjectId];
    __weak typeof(self) weakSelf = self;
...
- (void)FBaddAction:(UIButton *)FBsender{
    ...
    if (FBUser) {
        FBWardrobeAddViewController *FBaddVC = [[FBWardrobeAddViewController alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    FBWardrobeModel *FBmodel = self.FBDataArray[indexPath.row];
    FBWardrobeTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"FBWardrobeTableViewCell" forIndexPath:indexPath];
    cell.FBWardrobemodel = FBmodel;
...
- (UITableView *)FBmainTable{
    if (!_FBmainTable) {
        ...
        [_FBmainTable registerClass:[FBWardrobeTableViewCell class] forCellReuseIdentifier:@"FBWardrobeTableViewCell"];
