//
//  PFRecordDetailsViewController.m
//  PocketFootballScore
//
...
@interface PFRecordDetailsViewController ()<UITableViewDelegate,UITableViewDataSource>
@property(nonatomic, strong)NSMutableArray *PFDataArray;
@property(nonatomic, strong)UITableView *PFmainTable;
@end

@implementation PFRecordDetailsViewController
        ...
        if (error) {
            ...
            if (array.count) {
                for (int i = (int)(array.count - 1); i > -1; i--) {
                    ...
                    NSString *endTimeString = [obj objectForKey:@"PFendTimeString"];
                    NSString *dateSelectedString = [PFUIUtilities PFformattedTimeStringWithDate:self.dateSelected format:@"yyyy/MM/dd"];
                ...
                if (!self.PFDataArray.count) {
                    [MBProgressHUD PFshowReminderText:NSLocalizedString(@"无数据", nil)];
                }
                [weakSelf.PFmainTable reloadData];
                }else{
