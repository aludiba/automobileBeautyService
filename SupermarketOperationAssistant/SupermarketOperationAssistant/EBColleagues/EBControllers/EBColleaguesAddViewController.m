//
//  EBColleaguesAddViewController.m
...
#import "EBColleaguesAddTableViewCell.h"
#import "EBColleaguesAddModel.h"
#import "EBColleaguesViewController.h"
#import "EBColleaguesModel.h"
@interface EBColleaguesAddViewController ()<UITableViewDataSource,UITableViewDelegate>
@property(nonatomic, strong)EBColleaguesModel *EBModel;
...
@implementation EBColleaguesAddViewController
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (void)EBsetContentView{
    EBColleaguesAddModel *EBnameViewModel = [[EBColleaguesAddModel alloc] init];
    EBnameViewModel.EBTitle = NSLocalizedString(@"姓名:", nil);
    EBnameViewModel.EBDefault = NSLocalizedString(@"请输入姓名", nil);
    [self.EBViewDataArray addObject:EBnameViewModel];
    
    EBColleaguesAddModel *EBpositionViewModel = [[EBColleaguesAddModel alloc] init];
    EBpositionViewModel.EBTitle = NSLocalizedString(@"职位:", nil);
    EBpositionViewModel.EBDefault = NSLocalizedString(@"请输入职位", nil);
    [self.EBViewDataArray addObject:EBpositionViewModel];
    
    EBColleaguesAddModel *EBwageViewModel = [[EBColleaguesAddModel alloc] init];
    EBwageViewModel.EBTitle = NSLocalizedString(@"工资:", nil);
    EBwageViewModel.EBDefault = NSLocalizedString(@"请输入工资", nil);
    [self.EBViewDataArray addObject:EBwageViewModel];
    
    EBColleaguesAddModel *EBentrytimeViewModel = [[EBColleaguesAddModel alloc] init];
    EBentrytimeViewModel.EBTitle = NSLocalizedString(@"入职时间:", nil);
    EBentrytimeViewModel.EBDefault = NSLocalizedString(@"请输入入职时间", nil);
    [self.EBViewDataArray addObject:EBentrytimeViewModel];
    
    [self.EBmainTable reloadData];
}
- (void)EBsaveAction:(UIButton *)sender{
    for (int i = 0; i < self.EBViewDataArray.count; i++) {
        EBColleaguesAddModel *EBviewModel = self.EBViewDataArray[i];
    ...
    EBColleaguesAddModel *EBnameviewModel = self.EBViewDataArray[0];
    self.EBModel.EBname = EBnameviewModel .EBContent;
    EBColleaguesAddModel *EBpositionviewModel = self.EBViewDataArray[1];
    self.EBModel.EBposition = EBpositionviewModel.EBContent;
    EBColleaguesAddModel *EBwageviewModel = self.EBViewDataArray[2];
    self.EBModel.EBwage = EBwageviewModel.EBContent;
    EBColleaguesAddModel *EBentrytimeviewModel = self.EBViewDataArray[3];
    self.EBModel.EBentrytime = EBentrytimeviewModel.EBContent;
    ...
    AVObject *EBdiary = [AVObject objectWithClassName:@"EBColleagues"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    EBColleaguesAddModel *EBViewModel = self.EBViewDataArray[indexPath.row];
    NSString *EBKMyCellID = @"EBColleaguesAddTableViewCell";
    NSString *EBcellID = [NSString stringWithFormat:@"%@%zd",EBKMyCellID, indexPath.row];
    EBColleaguesAddTableViewCell *EBcell = [tableView dequeueReusableCellWithIdentifier:EBcellID];
    if (!EBcell) {
        EBcell = [[EBColleaguesAddTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:EBcellID];
    }
    EBcell.EBViewmodel = EBViewModel;
    EBcell.selectionStyle = UITableViewCellSelectionStyleNone;
    __weak typeof(self) weakSelf = self;
    EBcell.EBeditBlock = ^(EBColleaguesAddTableViewCell * _Nonnull cell) {
        if ([EBViewModel.EBTitle hasPrefix:NSLocalizedString(@"入职时间:", nil)]) {
...
#pragma mark - 属性懒加载
- (EBColleaguesModel *)EBModel{
    if (!_EBModel) {
        _EBModel = [[EBColleaguesModel alloc] init];
    }
    return _EBModel;
}
