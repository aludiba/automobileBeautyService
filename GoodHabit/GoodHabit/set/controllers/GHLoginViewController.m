...
+ (GHLoginViewController *)shareInstance{
    static GHLoginViewController *client;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        client = [[GHLoginViewController allocWithZone:NULL] init];
    });
    return client;
}
- (void)setContent{
    ...
    make.top.equalTo(self.view).offset(GHHeightNavBar + 50);
...
- (void)btnClick:(UIButton *)sender{
    ...
    if (!self.account.length) {
        [MBProgressHUD GHshowReminderText:NSLocalizedString(@"请输入账号", nil)];
        return;
    }
    if (!self.password.length) {
        [MBProgressHUD GHshowReminderText:NSLocalizedString(@"请输入密码", nil)];
        return;
    }
    if (sender.tag == 100) {
        ...
        [bUser signUpInBackgroundWithBlock:^ (BOOL isSuccessful, NSError *error){
            if (isSuccessful){
                [MBProgressHUD GHshowReminderText:NSLocalizedString(@"注册成功", nil)];
                [BmobUser loginWithUsernameInBackground:self.account password:self.password block:^(BmobUser *user, NSError *error) {
                    if (user) {
                        GHTabBarViewController *tabVC = [GHTabBarViewController shareInstance];
                        tabVC.selectedIndex = 0;
...
- (UIView *)backView{
    if (!_backView) {
        ...
        _backView.backgroundColor = GHH_Color(242, 242, 242, 1);
