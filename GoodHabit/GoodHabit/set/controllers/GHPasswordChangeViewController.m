//
//  GHPasswordChangeViewController.m
//  GoodHabit
//
...
@interface GHPasswordChangeViewController ()<UITextFieldDelegate>
...
@implementation GHPasswordChangeViewController
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (void)btnClick:(UIButton *)sender{
    ...
    if (!self.oldPassword.length) {
        [MBProgressHUD GHshowReminderText:NSLocalizedString(@"请输入旧密码", nil)];
        return;
    }
    if (!self.currentPassword.length) {
        [MBProgressHUD GHshowReminderText:NSLocalizedString(@"请输入新密码", nil)];
        return;
    }
    if (!self.confirmPassword.length) {
        [MBProgressHUD GHshowReminderText:NSLocalizedString(@"请确认密码", nil)];
        return;
    }
    if (![self.currentPassword isEqualToString:self.confirmPassword]) {
        [MBProgressHUD GHshowReminderText:NSLocalizedString(@"两次密码输入不一致", nil)];
        return;
    }
        ...
        [user updateCurrentUserPasswordWithOldPassword:self.oldPassword newPassword:self.confirmPassword block:^(BOOL isSuccessful, NSError *error) {
            if (isSuccessful) {
                ...
                [BmobUser loginInbackgroundWithAccount:name andPassword:self.confirmPassword block:^(BmobUser *user, NSError *error) {
                    if (error) {
                        [MBProgressHUD GHshowReminderText:[NSString stringWithFormat:@"%@",[error description]]];
                    } else {
                        [MBProgressHUD GHshowReminderText:NSLocalizedString(@"密码修改成功", nil)];
