...
@interface QSsmokingPlanClockView()<UITextFieldDelegate>
@property(nonatomic, strong)UIView *backView;
@property(nonatomic, strong)UIView *contentView;
@property(nonatomic, strong)UIButton *saveButton;
@end

@implementation QSsmokingPlanClockView
- (instancetype)init
{
    self = [super init];
    if (self) {
...
- (void)btnClick{
    self.hidden = YES;
    if (self.QSsmokingPlanClockSaveB) {
        self.QSsmokingPlanClockSaveB(self);
    }
}
...
- (UIView *)backView{
    if (!_backView) {
        ...
        _backView.backgroundColor = QSH_Color(0, 0, 0, 0.5);
    }
    return _backView;
}
- (UIView *)contentView{
...
- (UITextField *)contentField{
    if (!_contentField) {
        ...
        _contentField.placeholder = NSLocalizedString(@"   今天抽了几包烟", nil);
