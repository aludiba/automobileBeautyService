//
//  QSsmokingPlanDateTableViewCell.m
//  wineAwake
//
...
@interface QSsmokingPlanDateTableViewCell()<UITextFieldDelegate>
...
@end
@implementation QSsmokingPlanDateTableViewCell
...
- (void)setModel:(QSsmokingPlanModel *)model{
    _model = model;
    self.titleLbl.text = _model.QStitle;
    if (_model.QScontent.length) {
        self.editText.text = _model.QScontent;
    }else{
...
- (void)dateResult:(UIBarButtonItem *)sender{
    if (sender.tag == 100) {
        ...
        if (dateStr.length) {
            self.model.QScontent = dateStr;
        }else{
            self.editText.placeholder = _model.QSdefaultString;
        }
        if (self.QSsmokingPlanDateB) {
            self.QSsmokingPlanDateB(self);
        }
...
- (UIDatePicker *)datePicker{
    if (!_datePicker) {
        ...
        if (!language) {
            language = [QSHBTool getPreferredLanguage];
...
- (UIToolbar *)toolbar{
    if (!_toolbar) {
        ...
        toolbar.barTintColor = QSH_Color(242, 242, 242, 1);
        //设置工具条的frame
        toolbar.frame = CGRectMake(0, 0, QSWIDTH, 44);
        //给工具条添加按钮
...
- (UIImageView *)rightArrowImgView{
    if (!_rightArrowImgView) {
        _rightArrowImgView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"QS_arrow_right"]];
    }
...
- (UIView *)line{
    if (!_line) {
        ...
        _line.backgroundColor = QSH_Color(242, 242, 242, 1);
    }
    return _line;
}
@end
